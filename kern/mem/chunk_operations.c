...
void allocate_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	if(virtual_address > e->env_limit){
	    int pageIndex = (virtual_address - e->env_limit - PAGE_SIZE) / PAGE_SIZE;
	    envChunkSz[pageIndex] = pageCnt;
	}
	uint32 VA=virtual_address, *ptr_page_table = NULL;
...
{
	int pageIndex = (virtual_address - e->env_limit - PAGE_SIZE) / PAGE_SIZE;
	uint32 pageCnt = envChunkSz[pageIndex];
	...
	for(int i = 0 ; i < pageCnt; i++){
		...
		env_page_ws_O1_invalidate(e,env_page_ws_get_address(VA));
	...
	//need to be revised
	struct WorkingSetElement * i , *next;
	for(i = LIST_FIRST(&(e->page_WS_list)); i != NULL; i = next)
	{
