...
//	return (void*)-1 ;
	...
	// what var holds the env segment break and hard limit

	if(increment == 0){
		return (void *)env->env_seg_break;
	}else if(increment > 0 ){
		uint32 newSbrk = env->env_seg_break + increment;
		uint32 tmpSbrk = ROUNDUP(env->env_seg_break, PAGE_SIZE);
		if(newSbrk >= env->env_limit){
			return (void *)-1;
		}

		if(tmpSbrk <= newSbrk){
			sys_allocate_user_mem(tmpSbrk, MAX(1, newSbrk - tmpSbrk));
		}

		uint32 tmp = env->env_seg_break ;
		env->env_seg_break = ROUNDUP(newSbrk, PAGE_SIZE);
		return (void *)tmp;
	}
	else if(increment < 0){
		uint32 n;
		uint32 tmp = ROUNDDOWN(env->env_seg_break, PAGE_SIZE);
		env->env_seg_break += increment ;
		n = (ROUNDDOWN(tmp, PAGE_SIZE) - ROUNDDOWN(env->env_seg_break, PAGE_SIZE)) / PAGE_SIZE;
		if(n < 0)
			n = 0;

		free_user_mem(env, ROUNDUP(env->env_seg_break, PAGE_SIZE), n * PAGE_SIZE);
		return (void *)env->env_seg_break;
	}
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_sbrk_limit :
		...
		case SYS_free_user_mem_limit :
			  if((void*)a1==NULL || a1+a2>USER_LIMIT || a1<0){
				  cprintf("999999999");
		...
		case SYS_allocate_user_mem_limit :
			  if((void*)a1==NULL || a1+a2>USER_LIMIT || a1<0){
				  cprintf("656565656");
				sched_kill_env(curenv->env_id);
		      }
		...
		case SYS_Check_Hard_Limit :
				 return curenv->env_limit ;
				 break ;

		case SYS_get_page_permission:{
			int perms = pt_get_page_permissions(curenv->env_page_directory, a1);
			if(perms == -1)
				perms = 0;
