...
void fault_handler(struct Trapframe *tf)
{
	...
	else
	{
		if (userTrap)
		{
		   ...
		   if ((!((PERM_MARKED & permissions)==PERM_MARKED) )&& (fault_va< USER_HEAP_MAX  && fault_va>= USER_HEAP_START))
			 {
		  ...
		  if(((PERM_PRESENT&permissions)==PERM_PRESENT))
		  	  {
				sched_kill_env(curenv->env_id);
		  	  }
		 /* if(((PERM_PRESENT&permissions)==PERM_PRESENT) && (fault_va> USER_HEAP_MAX || fault_va< USER_HEAP_START) )
		  	  {
			    //tprlru1 killed
				sched_kill_env(curenv->env_id);
			  }*/
