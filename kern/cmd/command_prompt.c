...
int process_command(int number_of_arguments, char** arguments)
{
    ...
    for(int i=0 ; i<NUM_OF_COMMANDS; i++){
        if(strcmp(commands[i].name, arguments[0]) == 0){
            ...
            if(commands[i].num_of_args==number_of_arguments-1 ||commands[i].num_of_args==-1 ){
                flag=1;     //command found with valid arguments--> case 4
               temp=i;
            }
            else{
  ...
  if(found&&flag){
      ...
      bool check=0;
      bool matched_command=0;
      for(int i=0 ; i< NUM_OF_COMMANDS ; i++){
          check= subsequence_checker(commands[i].name, arguments[0]);
          if(check){
              matched_command=1;
              LIST_INSERT_HEAD(&foundCommands, &commands[i]);
          }
...
bool subsequence_checker(char * s, char* x){
    bool length[1000] = {};
    length[0] = 1;

    for(int i = 0; i < strlen(s); i++){
    	for (int j = strlen(x) - 1; j >= 0; j--){
            if(s[i] == x[j] && length[j] == 1)
                length[j + 1] = 1;
        }
    }

    bool ret = length[strlen(x)];
    return ret;
}
